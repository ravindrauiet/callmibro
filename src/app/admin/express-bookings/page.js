  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h2 className="text-2xl font-bold text-white">Express Bookings Management</h2>
      ...
      <div className="relative">
        <input
          type="text"
          placeholder="Search express bookings..."
          value={searchTerm}
              ...
              {filteredBookings.length > 0 ? (
                filteredBookings.map((booking) => (
                  <tr key={booking.id} className="hover:bg-[#222] transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                      <div className="font-medium text-[#e60012]">{booking.contactInfo?.phone || 'N/A'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                      <div>
                        <div>{booking.createdAt ? formatDate(booking.createdAt) : 'N/A'}</div>
                        <div className="text-gray-400 text-xs">Express Booking</div>
                      </div>
                    </td>
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-4 text-center text-sm text-gray-400">
                    {searchTerm 
                      ? 'No express bookings found matching your search' 
                      : filterStatus !== 'all' 
                        ? `No ${filterStatus} express bookings found` 
      ...
      {showStatusModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-[#1a1a1a] rounded-lg p-6 max-w-md w-full">
            <h3 className="text-xl font-medium text-white mb-4">Update Express Booking Status</h3>
