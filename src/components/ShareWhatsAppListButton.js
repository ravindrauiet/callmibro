...
export default function ShareWhatsAppListButton({ shopName, contactNumber, inventory, buttonText = 'WhatsApp (as List)', className = '' }) {
  const handleClick = () => {
    const inventoryList = inventory.map(item => 
      `• ${item.name} (${item.brand}) - ₹${item.price} - ${item.quantity} available`
    ).join('\n')
    const message = `${shopName} - Available Inventory:\n\n${inventoryList}${contactNumber ? `\n\nContact: ${contactNumber}` : ''}`
    const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(message)}`
    window.open(whatsappUrl, '_blank')
  }
  return (
    <button onClick={handleClick} className={`flex items-center px-3 py-2 bg-green-700 text-white rounded-lg hover:bg-green-800 transition-colors duration-200 ${className}`}>
