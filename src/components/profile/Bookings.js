      ...
      {bookings.length > 0 ? (
        <div className="space-y-6">
          {bookings.map(booking => (
            <div key={booking.id} className="rounded-xl p-5 hover:shadow-md transition-all" style={{ 
              ...
              <div className="flex flex-col md:flex-row justify-between mb-4">
                <div>
                  <h4 className="font-medium text-lg" style={{ color: 'var(--text-main)' }}>{booking.serviceName || 'Service'}</h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div className="p-3 rounded-lg" style={{ backgroundColor: isDarkMode ? '#161616' : 'var(--panel-charcoal)' }}>
                  <p style={{ color: 'var(--text-secondary)', marginBottom: '4px' }}>Service Type</p>
                  <p className="font-medium" style={{ color: 'var(--text-main)' }}>{booking.serviceType || 'N/A'}</p>
                </div>
                
                <div className="p-3 rounded-lg" style={{ backgroundColor: isDarkMode ? '#161616' : 'var(--panel-charcoal)' }}>
                  <p style={{ color: 'var(--text-secondary)', marginBottom: '4px' }}>Address</p>
                  <p className="font-medium truncate" style={{ color: 'var(--text-main)' }}>{booking.address || 'Address not set'}</p>
                </div>
                
                <div className="p-3 rounded-lg" style={{ backgroundColor: isDarkMode ? '#161616' : 'var(--panel-charcoal)' }}>
                  <p style={{ color: 'var(--text-secondary)', marginBottom: '4px' }}>Price</p>
                  <p className="font-medium" style={{ color: 'var(--text-main)' }}>{booking.price ? `₹${booking.price}` : 'To be determined'}</p>
                </div>
              </div>
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                <button 
                  ...
                  className="text-sm text-white transition-all px-4 py-2 rounded-lg ml-auto"
        ...
        <div className="text-center py-12 rounded-xl" style={{ 
          ...
          <h4 className="text-xl font-medium mb-2" style={{ color: 'var(--text-main)' }}>No bookings yet</h4>
          <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>You haven't made any service bookings yet.</p>
          <button 
