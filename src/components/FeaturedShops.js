  ...
  return (
    <section id="featured-shops-section" className="py-16 px-4 sm:px-8">
      <div className="container mx-auto max-w-6xl">
        ...
        {/* Shops Grid */}
        {loading ? (
          <div className="overflow-x-auto pb-4 -mx-4 px-4 hide-scrollbar snap-x snap-mandatory">
            <div className="flex gap-4 min-w-max">
              {[1, 2, 3, 4].map(i => (
                <div 
                  key={i}
                  className="w-[280px] flex-shrink-0 snap-start rounded-lg border animate-pulse"
                  style={{ 
                    height: '200px',
          ...
          <div className="overflow-x-auto pb-4 -mx-4 px-4 hide-scrollbar snap-x snap-mandatory">
            <div className="flex gap-4 min-w-max">
              {shops.map((shop, index) => (
                <div
                  key={shop.id}
                  className={`w-[280px] flex-shrink-0 snap-start rounded-lg border overflow-hidden transform transition-all duration-700 ${
                    isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
                  } hover:shadow-lg hover:shadow-[#e60012]/10 group`}
                  style={{ 
                ...
                >
                  ...
                  <div className="relative h-32 overflow-hidden">
                    ...
                    {shop.featured && (
                      <div className="absolute top-2 right-2 bg-[#e60012] text-white text-xs font-bold px-2 py-1 rounded-full">
                        ⭐ Featured
                      </div>
                    )}
                    {shop.verified && (
                      <div className="absolute top-2 left-2 bg-blue-500/80 text-white text-xs px-2 py-1 rounded-full flex items-center">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3">
                      <h3 className="text-white text-sm font-bold truncate">{shop.shopName}</h3>
                      <p className="text-white/80 text-xs truncate">{shop.shopAddress}</p>
                    </div>
                  </div>
                  ...
                  <div className="p-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>{shop.shopCategory}</span>
                    ...
                    <div className="mb-3">
                      <div className="flex flex-wrap gap-1">
                        {shop.services && shop.services.slice(0, 2).map((service, idx) => (
                          <span 
                            key={idx} 
                        ...
                        {shop.services && shop.services.length > 2 && (
                          <span 
                            className="px-2 py-0.5 text-xs rounded-full" 
                            style={{ 
                              background: 'var(--panel-charcoal)',
                    ...
                    <div className="flex items-center mb-3">
                      <div className="w-6 h-6 rounded-full bg-[#e60012] flex items-center justify-center text-white text-xs font-bold mr-2">
                        {shop.ownerName?.charAt(0) || 'O'}
                      </div>
                      <div>
                        <p className="text-xs font-medium">Owned by</p>
                        <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>{shop.ownerName}</p>
                      </div>
                    </div>
                    ...
                    <Link 
                      ...
                      className="w-full py-2 text-center block rounded-lg transition-colors border text-xs font-medium hover:scale-105"
                      style={{ 
